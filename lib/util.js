...
function getWmic() {
  if (os.type() === 'Windows_NT' && !wmicPath) {
    ...
    if (!fs.existsSync(wmicPath)) {
      try {
        const wmicPathArray = execSyncWin('WHERE WMIC').toString().split('\r\n');
...
function powerShell(cmd) {
  ...
  if (_psPersistent) {
    ...
    return new Promise((resolve) => {
      process.nextTick(() => {
        try {
          const child = spawn(_powerShell, ['-NoProfile', '-NoLogo', '-InputFormat', 'Text', '-ExecutionPolicy', 'Unrestricted', '-Command', _psToUTF8 + cmd], {
            stdio: 'pipe',
            windowsHide: true,
            maxBuffer: 1024 * 20000,
            encoding: 'UTF-8',
...
function getCodepage() {
  if (_windows) {
    if (!codepage) {
      try {
        const stdout = execSync('chcp', { windowsHide: true, encoding: 'utf8' });
        const lines = stdout.toString().split('\r\n');
        const parts = lines[0].split(':');
        codepage = parts.length > 1 ? parts[1].replace('.', '').trim() : '';
        if (!codepage) {
          codepage = '936'; // 默认为中文 GBK
        }
      } catch (err) {
        codepage = '936';
      }
...
function smartMonToolsInstalled() {
  ...
  if (_windows) {
    try {
      const pathArray = execSyncWin('WHERE smartctl 2>nul').toString().split('\r\n');
...
exports.execSyncWin = execSyncWin;
