...
function getDefaultNetworkInterface() {
  ...
  try {
    ...
    if (_windows) {
      ...
      let defaultIp = '';
      const cmd = 'netstat -r';
      const result = util.execSyncWin(cmd);
...
function getWindowsDNSsuffixes() {
  ...
  try {
    const ipconfig = _windows ? util.execSyncWin('ipconfig /all').toString() : execSync('ipconfig /all', util.execOptsWin);
...
function getWindowsWiredProfilesInformation() {
  try {
    const result = util.execSyncWin('netsh lan show profiles');
    const profileList = result.toString().split('\r\nProfile on interface');
    return profileList;
  } catch (error) {
...
function getWindowsWirelessIfaceSSID(interfaceName) {
  try {
    const result = util.execSyncWin(`netsh wlan show  interface name="${interfaceName}" | findstr "SSID"`);
    const SSID = result.toString().split('\r\n').shift();
  ...
  if (connectionType == 'wired' && ifaces.length > 0) {
    ...
    try {
      ...
      if (SSID !== 'Unknown') {
        ...
        i8021xState = util.execSyncWin(`netsh wlan show profiles "${ifaceSanitized}" | findstr "802.1X"`).toString();
        i8021xProtocol = util.execSyncWin(`netsh wlan show profiles "${ifaceSanitized}" | findstr "EAP"`).toString();
      }
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        let cmd = 'netstat -nao';
        try {
          util.execWin(cmd, function (error, stdout) {
            if (!error) {
...
function networkGatewayDefault(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          util.execWin('netstat -r', function (error, stdout) {
