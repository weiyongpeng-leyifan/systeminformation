...
function getFQDN() {
  ...
  if (_windows) {
    try {
      const stdout = util.execSyncWin('echo %COMPUTERNAME%.%USERDNSDOMAIN%');
...
function isUefiWindows() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      try {
        util.execWin('findstr /C:"Detected boot environment" "%windir%\\Panther\\setupact.log"', function (error, stdout) {
          if (!error) {
            const line = stdout.toString().split('\n\r')[0];
            return resolve(line.toLowerCase().indexOf('efi') >= 0);
          } else {
            util.execWin('echo %firmware_type%', function (error, stdout) {
              if (!error) {
...
echo -n "hardware: "; cat /sys/class/dmi/id/product_uuid 2> /dev/null; echo;`;
      ...
      if (_windows) {
        ...
        util.powerShell('Get-CimInstance Win32_ComputerSystemProduct | select UUID | fl').then((stdout) => {
          ...
          util.execWin(`${sysdir}\\reg query "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Cryptography" /v MachineGuid`, function (error, stdout) {
